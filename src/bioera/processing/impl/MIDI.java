/* MIDI.java v 1.0.9   11/6/04 7:15 PM
 *
 ...
 */
public final class MIDI extends Element {
...
public final void process() throws java.lang.Exception {
	...
	if (prevNote != -1) {
		message.setMessage(ShortMessage.NOTE_OFF, channel, prevNote, velocity);
	...
	message.setMessage(ShortMessage.NOTE_ON, channel, n, vol);
	midiReceiver.send(message, -1);
...
public void reinit()  throws Exception {
	...
	if (midiDevice.getSelectedIndex() > 0 && midiDevice.getSelectedIndex() < midiDevice.getItems().length) {
		...
		//System.out.println("synth=" + synth.getClass());
		//c[0].noteOn(44, 100);
		//for (int k = 0; k < 4; k++){
			//for (int i = 0; i < 100; i++){
				//c[0].setPitchBend(8000 + i * 80);
				//Thread.sleep(1);
			//}
			//for (int i = 0; i < 100; i++){
				//c[0].setPitchBend(16000 - i * 80);
				//Thread.sleep(1);
			//}
		//}
	}
