/* JHorizontalScalarChart.java v 1.0.9   11/6/04 7:15 PM
 *
 ...
 */
public class JHorizontalScalarChart extends JHorizontalChart {
	// Internal values
	...
	public boolean joinPoints = true;
        private Color niceblue = new Color( 97, 155, 211 );
        private Color nicegreen = new Color( 0, 197, 144 );
        private Color nicered = new Color( 207, 0, 99 );
        private Color nicepurple = new Color( 141, 101, 210 );
        private Color niceorange = new Color( 255, 82, 0);
...
//    public void pushDataChannel(
    ...
    public void pushStreams(int value, int xspot, int stream, boolean first, int mode) {
        ...
        if (stream==1){gr.setColor(niceblue);}
        if (stream==2){if (secondOfDual=true){gr.setColor(niceblue.brighter());}
            else{gr.setColor(nicegreen);}        }
        if (stream==3){gr.setColor(nicered);}
        if (stream==4){if (secondOfDual=true){gr.setColor(nicered.brighter());}
            else{gr.setColor(nicepurple);}        }
        if (stream==5){gr.setColor(Color.orange);}
        if (stream==6){if (secondOfDual=true){gr.setColor(Color.orange.brighter());}
            else{gr.setColor(Color.PINK);}        }
        if (stream==7){gr.setColor(Color.cyan);}
        if (stream==8){if (secondOfDual=true){gr.setColor(Color.cyan.brighter());}
