/* ReplaceOldNames.java v 1.0.9   11/6/04 7:15 PM
 *
...
public final static void main(String args[]) {
	try {
		new ReplaceOldNames().start(args);
...
public void processFile(File f) throws Exception {
	...
	s = Tools.changeSubstr(s1 = s, "jbiopro", "bioera");
	if (!s.equals(s1))
		any = true;
	s = Tools.changeSubstr(s1 = s, "jbrain", "bioera");
	if (!s.equals(s1))
		any = true;
	s = Tools.changeSubstr(s1 = s, "JBrain", "BioEra");	
	if (!s.equals(s1))
	...
	if (!s.equals(s1))
		any = true;
	s = Tools.changeSubstr(s1 = s, "BioPro", "BioEra");
	if (!s.equals(s1))
		any = true;
	s = Tools.changeSubstr(s1 = s, "Biopro", "BioEra");
	if (!s.equals(s1))
