/* SourceChanger.java v 1.0.9   11/6/04 7:15 PM
 *
...
public class SourceChanger {
	...
	protected static boolean debug = Debugger.get("source.changer");
/**
 * Debugger constructor comment.
 */
public SourceChanger() {
	super();
	debug = true;
}
/**
 ...
 * Debugger constructor comment.
 */
private void convertPropertable(File f, String path) throws Exception {
	String cname = path + "." + f.getName().substring(0, f.getName().length() - ".java".length());
	...
	try {
		...
		System.out.println("Class not loaded: " + cname);
		return;
	}
	...
	String content = new String(Tools.readFile(f, 1000000));

	bioera.processing.ElementProperty props[] = bioera.processing.ProcessingTools.getElementProperties((bioera.processing.Propertable)c.newInstance());
	
	String newStr = "getPropertyNames() {\r\n"
		+ "\tString s[] = {\r\n";
	...
	try {
		Method m = c.getMethod("getPropertyNames", new Class[]{});
		System.out.println("method exists:");
		content = Tools.changeSubstr(content, 
	"getPropertyNames() {\r\n\treturn null;\r\n}",
	newStr);
		System.out.println("string replaced = ''");
	} catch (Exception e) {
		System.out.println("              method needs to be added");
	}
		
	
	Tools.writeFile(f, content.getBytes());
...
/*	
	...
	sb.append("public final void sendChangePropertyEvent(String fieldName) {" + CR);
...
private void processFile(File f, String path) throws Exception {
	...
	convertPropertable(f, path);
}
/**
 * Debugger constructor comment.
 */
private void processFolder(File srcFolder, String path) throws Exception {
	...
	if (list != null) {
		for (int i = 0; i < list.length; i++){
			...
			if (f.isFile()) {
				...
				if (recurse)
					processFolder(new File(srcFolder, f.getName()), path + "." + f.getName());
			}
		}
	}
}
...
public void start(String[] args) throws Exception {
	...
	processFolder(new File(srcDir), "bioera");
